...
void GameEngine::startupPhase()
{
	...
	for (int i = 0; i < MainMap->NumOfCountries(); i++)
	{
		...
		MainMap->ReturnListOfCountries().at(randomizedIDs.at(i))->setState(new_state);
	...
	case 2:
		for (int i = 0; i < 2; i++)
		{
			...
			State new_state;
			new_state.current_state = State_enum::SETUP_PHASE_RECEIVE_REINFORCEMENTS;
	...
	while (ordersLeft > 0)
	{
		...
		for (auto &player : ListOfPlayers)
		{
			...
			if (player->ListOfOrders->peek()->getName() == "DEPLOY")
			{
				...
				new_phase_state.executed_order_name = player->ListOfOrders->peek()->getName();

				// State new_game_stats_state;
				// new_game_stats_state.current_state = State_enum::TERRITORY_CONQUERED;
				// new_game_stats_state.executed_order_name = player->ListOfOrders->peek()->getName();
...
void GameEngine::displayStatistics() {
	...
	for (auto *player : ListOfPlayers) {
		...
		player_territories = MainMap->ReturnListOfCountriesOwnedByPlayer(player->PID).size();
		std::cout << "Name: " << player->PID << std::endl;
		std::cout << std::fixed << std::showpoint << std::setprecision(2);
		std::cout << "\tOwned Territories: " << player_territories 
				<< " ( " << (double)player_territories / total_territories * 100
				<< "% of total)" << std::endl;
		std::cout << "--------------------------" << std::endl;
	}
}