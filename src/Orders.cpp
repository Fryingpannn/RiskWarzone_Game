...
// returns: nullptr if list is empty, otherwise returns a pointer to
//          the next priority element (needs to be explicitly deleted)
...
bool Deploy::validate() {
  ...
  if (target == nullptr || target->OwnedBy != playerID || armyNb <= 0)
    return false;
  else
    return true;
}
...
bool Advance::validate() {
  ...
  if (src == nullptr || armyNb <= 0 || src->OwnedBy != playerID ||
  ...
  if (validate()) {
    ...
    // if target territory not owned by user, attack initiated
      ...
      if (target->Armies <= 0 && armyNb <= 0) {
        target->Armies = 0;
        std::cout << "[Valid] 1 Advance order executed. Attack failed: "
                  << playerID << " lost their sent armies." << std::endl;
        success_result = false;
      }
      // IF ATTACKER WON --
        ...
        if (target->OwnedBy != "neutral") {
          target->PlayerOwned->removeTerritory(*target);
        }
...
bool Bomb::validate() {
  ...
  if (target == nullptr || target->OwnedBy == playerID ||
      !current->set.empty() &&
          current->set.find(target->OwnedBy) != current->set.end())
    return false;
  else
...
bool Blockade::validate() {
  std::cout << " Validating order..." << std::endl;
  if (src == nullptr || src->OwnedBy != playerID ||
      src->PlayerOwned->PID != playerID)
    return false;
  else
    return true;
}
...
bool Blockade::execute() {
  ...
  if (validate()) {
    ...
    if (src->PlayerOwned == nullptr) {
      std::cout << "Blockade Error: Player owned attribute of source territory "
                   "is null."
                << std::endl;
    }
    else {
  ...
  if (validate()) {
    ...
    // if target territory not owned by user, attack initiated
      ...
      if (target->Armies <= 0 && armyNb <= 0) {
        ...
        target->Armies = 0;
        std::cout << "[Valid] 1 Airlift order executed. Attack unsuccessful:"
                  << playerID << " lost their armies." << std::endl;
        success_result = false;
      } else if (target->Armies <= 0) {
        // if attacker won
        target->Armies = armyNb;
...
bool Negotiate::validate() {
  ...
  if (enemy == nullptr || enemy->PID == playerID)
    return false;
  else
    return true;
}
...
std::ostream& Reinforcement::doprint(std::ostream& out) {
  ...
  // if order has been executed, also print out effect of order
  if (getExecuted())
    out << " [Reinforcement] Reinforcement successful. 5 army units given to "
           "the player"
        << std::endl;
